...
router.get('/explwrite/:ProfId/:StudentId', async (req, res) => {
    ...
    res.render('cssys/guidance/student/explanatory_write', {ProfId: prof.id, StudentId: req.params.StudentId, Profname: prof.User.name});
});
...
router.post('/modiProf', async (req, res) => { // 수정 취소 없이 바로 수정
  await models.Student.update({state: 0, ProfId: null}, {where: {id: req.body.StudentId}});
  await models.GPermissionLog.create({resorreq: 'req', state: 0, text:req.body.text, ProfId: req.body.ProfId, StudentId: req.body.StudentId});
  res.send({
      result: true,
  });
});
