...
router.get('/main', function(req, res, next) {
    ...
    async.series({
        recentLog: function(callback) {
            models.GPermissionLog.findAll({
                where: {
                    resorreq: 'res',
                    state: 1  
                },
                ...
                callback(null, recentLog);
            })
        },
        cancelLog: function(callback) {
            models.GPermissionLog.findAll({
                where: {
                    resorreq: 'res',
                    state: 0  
    ...
    function(err, results) {
        const Log = results.recentLog.concat(results.cancelLog);
        ...
        res.render('cssys/guidance/admin/main', {
            ...
            applying_students: results.students.filter(student => student.Student && student.Student.state == 1),
            completed_students: results.students.filter(student => student.Student && student.Student.state == 2),
            today_applying_students: results.students.filter(student => student.Student && student.Student.state == 1 && student.Student.updatedAt.toLocaleDateString() == today),
            today_completed_students: results.students.filter(student => student.Student && student.Student.state == 2 && student.Student.updatedAt.toLocaleDateString() == today),
            profs: results.profs,
            //recentLog: results.recentLog,
            //cancelLog: results.cancelLog
            recentLog: Log
        });
    });
});
        ...
        if (user) {
            if (req.body.delete) {
                ...
                if (file.isFileSizeLimit) {
                    ...
                    var file_path = path.join(config.cssys.upload_path, 'guidance/' + req.body.upload, file_name);
                    mkdirp(path.join(config.cssys.upload_path, 'guidance/' + req.body.upload), function(err) {
                        if (!err) {
    ...
    if (req.body.id) { // 수정일경우
        models.User.findOne({
            ...
            if (user !== null) {
                ...
                user.updateAttributes(req.body).then(async function(user) {
                    ...
                    async.series([
                        ...
                        function(err, results) {
                            ...
                            models.User.findOne({
                                ...
                                if (user === null) { // 아이디 없을시 생성
                                    models.User.create(userTmp).then(async function(user) {
                                        userTmp.Student.UserId = user.id;
                                        ...
                                        if (!result1 || !result2) {
                                            if (result1) {
                                                await models.Student.destroy({ where: { Userid: user.id} })
                                            }
                                            if (result2) {
                                                await models_w.Student.destroy({ where: { Userid: user.id} })
                                            }
                                    ...
                                    user.save().then(function(user) {
                                        user.Student.save().then(async function(student) {
                                            await models.Student.update({ // state, id, ProfId, UserId update 안함
                                                term: student.term,
    ...
    models.User.findAll({
        ...
        users.forEach(function(user) {
            ...
            ["oath", "proposal", "midreport", "finalreport", "paperwork", "presentation", "conference"].forEach(function(index) {
                if (user.Student[index]) user.Student[index].dataValues.link = '/cssys/guidance/ajax/file/download/' + index + '/' + path.basename(user.Student[index].path);
            });
        });
        res.send({
            aaData: users
...
router.get('/prof/:prof_id/status', async (req, res) => { // 특정 교수 현황 조회
    ...
    const Nstudents = await models.Student.findAll({ // 학생 목록
        where: {
            ProfId: null,
            state: 1
        },
    ...
    const Nstu = await Promise.all(Nstudents.map(async (stu) => {
        const Log = await models.GPermissionLog.findOne({ // 최신 로그가 우리가 원하는 교수에게 신청한 경우 check
            where: {
                resorreq: 'req',
                state: 1,
                ProfId: Profid
    ...
    let index = 1;
    result.aaData = await Promise.all( result.aaData.map(async (i) => {i.dataValues.index = index++; return Promise.resolve(i);} ));
    res.send(result);
});
...
router.get('/student/:student_id/status', async (req, res) => {
    ...
    let data = {}
    if (!!student) { // 초기 학생
        if(!student.ProfId && student.state == 0) { // 신청하기 전 상태
            data.result = true;
            data.message = "This student did not apply any professor";
        }
        else if (!student.ProfId && student.state == 1) { // 교수한테 신청한 상황(배정x)
            const Prof = await models.GPermissionLog.findAll({ // findOne이 가능할려나
                where: {
        ...
        else if (!!student.ProfId && student.state == 2 ) { // 배정된 상황
        ...
        } else if (!student.ProfId && student.state == 1) { // 취소한 상황
            data.student = student.dataValues;
    ...
    } else { // 학생 계정 처음 만들어지고 나서 or 교수, 어드민인 경우
        data.result = true;
...
router.get('/guidance_status_excel', async (req, res) => {
    ...
    const Student = await models.User.findAll({
        ...
        order: [ // (재학, 휴학, 수료, 졸업) -> (미배정, 신청중, 배정) -> (학기 순) -> 이름 순
    ...
    for (let i = 0; i<Student.length; i++) {
        ...
        if (user.Student) { // user, g_student table이 동시에 있는 경우에만
            if (!!user.Student.Prof && user.Student.state == 2) { // 배정 확정인 경우
                // 현재
            ...
            else if (!user.Student.Prof && user.Student.state == 1) { // 신청한 상태인 경우
                row[3] = 1; // 대기 중
                /*
                ...
                */
            } else if (!user.Student.Prof && user.Student.state == 0){ // 신청 안한 경우
                row[3] = 0; // 미배정
            } else {
                row[3] = 3; // 오류
            }
        ...
        */
        ...
        data.push([
            user.ids, // 학번
            user.name, // 이름
            ['전자전기공학부', '컴퓨터공학과', '반도체시스템공학과', '소프트웨어학과', '정보통신대학', '인터랙션사이언스학과'][user.major], // 학과
    ...
    res.setHeader('Content-disposition', 'attachment; filename=guidance_student_list(' + moment().format("YYYYMMDDHHmmss") + ').xlsx');
    res.setHeader('Content-type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.send(buffer);
});


router.post('/guidance_regi_excel', async (req, res) => {
    ...
    for (let i=0; i<obj[0].data.length; i++) {
        ...
        let resrow = ['', '', ''];
        // 0 순번, 1 학번, 2 사유
        if (!!data[0] && !!data[1]) { // 학번이랑 이름만 있는 경우에만
            // 1. 학번(ids)으로 조회
            ...
            if (!!User && !!User.dataValues.Student) {
                ...
                if (Dupnum.length == 0) {
                    ...
                    if (!!data[5] && !!data[6]) { // !!data[6] 교수 이름 있는 경우(일단 학과 안 적혀 있는 경우로만 코딩)
                        ...
                        if (!!Prof && !!Prof.dataValues.Prof) {
                            ...
                            if (User.dataValues.Student.state == 2) {
                                if ( Prof.dataValues.id != User.Student.Prof.UserId ) { // 교수님이 변경된 경우
                                    // 1. Student 학생의 state을 2로 고정(확정)
                                    await models.Student.update({state: 2, ProfId: Prof.dataValues.Prof.dataValues.id }, {where: {id: User.dataValues.Student.dataValues.id}});
                                    // 2. 로그 생성
                                    // 2-1. 기존 교수 취소 로그
                                    await models.GPermissionLog.create({resorreq: 'req', state: 0, text:'   관리자 권한으로 배정이 취소되었습니다.   ', ProfId: User.Student.Prof.dataValues.id, StudentId: User.dataValues.Student.dataValues.id});
                                    // 2-2. 새 교수 신청 로그
                                    await models.GPermissionLog.create({resorreq: 'req', state: 1, ProfId: Prof.dataValues.Prof.dataValues.id, StudentId: User.dataValues.Student.dataValues.id});
                                    // 2-3. 새 교수 수락 로그
                                    await models.GPermissionLog.create({resorreq: 'res', state: 1, ProfId: Prof.dataValues.Prof.dataValues.id, StudentId: User.dataValues.Student.dataValues.id});
                        ...
                        } else { // 교수가 prof 테이블에 존재하지 않는 경우
                            resrow[0] = data[0] // 학번
                            resrow[1] = data[1] // 이름
                            resrow[2] = `실패 (사유: ${data[5]}(${data[6]}) 교수가 존재하지 않습니다.)`
                            resExcel.push(resrow);
                        }
                    } else if (!data[5] && !!data[6]) { // 교수 이름을 적지 않은 경우
                        resrow[0] = data[0] // 학번
                        resrow[1] = data[1] // 이름
                        resrow[2] = `실패 (사유: 해당 교수의 성함을 적지 않았습니다.)`
                        resExcel.push(resrow);
                    } else if (!!data[5] && !data[6]) { // 교수 소속학과를 적지 않은 경우
                        resrow[0] = data[0] // 학번
                        resrow[1] = data[1] // 이름
        ...
        } else if (!data[0] && !!data[1]) { // 학생 학번을 적지 않은 경우
            resrow[1] = data[1] // 이름
            resrow[2] = `실패 (사유: 해당 학생의 학번을 적지 않았습니다.)`
            resExcel.push(resrow);
        } else if (!!data[0] && !data[1]) { // 학생 이름을 적지 않은 경우
            resrow[0] = data[0] // 학번
