...
router.all('*', function(req, res, next) {
    ...
    if (!req.session.ip) req.session.ip = (req.headers['x-forwarded-for'] || req.socket.remoteAddress || req.socket.remoteAddress || (req.socket ? req.socket.remoteAddress : null)).split(",")[0];
...
router.post('/login', async function(req, res, next) {
    try {
        var user = await models.User.findOne({ // 유저 검색
            where: {
                ids: req.body.ids,
    ...
    try {
        ...
        if (board !== null) {
            var data = await models.sequelize.query(
                ...
                'where BoardPost.BoardId=:BoardId and BoardPost.id=:PostId', { // sql 인젝션 공격 방지
                    raw: true,
                    replacements: {
                        BoardId: board.id,
            ...
            if (data[0]) { // 글이 존재할경우
                ...
                if (!data[0].secret || data[0].secret && (req.session.user.type === 0 || data[0].UserId == req.session.user.id || (data[0].ParentUserId !== null && data[0].ParentUserId == req.session.user.id))) {
                    ...
                    if (data[0].notice) { // 공지사항 일경우
                        var prevData = await models.sequelize.query('select id,title from cssys_board_post where BoardId=' + board.id + ' and id>' + data[0].id + ' and notice order by id limit 0,1');
                    ...
                    var nextPost;
                    if (data[0].notice) { // 공지사항 일경우
                        var nextData = await models.sequelize.query('select Post.id,Post.title from ((select * from cssys_board_post where BoardId=' + board.id + ' and id<' + data[0].id + ' and notice order by id desc limit 0,1) union (select * from cssys_board_post where BoardId=' + board.id + ' and ParentId is null order by id desc limit 0,1)) as Post');
                    ...
                    await models.sequelize.query('update cssys_board_post set views=views+1 where BoardId=' + board.id + ' and id=' + data[0].id);
...
router.post('/ajax/board/write/:title', upload.fields([{name: 'file_1', maxCount: 1}, {name: 'file_2', maxCount: 1}]), async function(req, res, next) {
    try {
        var board = await models.Board.findOne({
            where: {
                title: req.params.title
...
router.post('/ajax/board/reply/:title/:id', upload.fields([{name: 'file_1', maxCount: 1}, {name: 'file_2', maxCount: 1}]), async function(req, res, next) {
    try {
        var board = await models.Board.findOne({
            where: {
                title: req.params.title
...
router.post('/ajax/board/modify/:title/:id', upload.fields([{name: 'file_1', maxCount: 1}, {name: 'file_2', maxCount: 1}]), async function(req, res, next) {
    try {
        var board = await models.Board.findOne({
            where: {
                title: req.params.title
