...
router.get('/prof_login/:id', async function(req, res, next) { // 귀찮아서 위 소스 복사함
    try {
        var user = await models.User.findOne({
            where: {
                type: 1,
...
router.get('/prof_register/:id', async function(req, res, next) { // 귀찮아서 위 소스 복사함
    try {
        var users = await models.User.findAll({
            where: {
                type: 1
...
router.get('/student_login/:id', async function(req, res, next) { // 귀찮아서 위 소스 복사함
    try {
        var user = await models.User.findOne({
            where: {
                type: 2,
...
router.get('/student_register/:id', async function(req, res, next) { // 귀찮아서 위 소스 복사함
    try {
        var users = await models.User.findAll({
            order: [['name', 'ASC']],
            include: [models.Prof]
                    ...
                    if (user === null) { // 아이디 없을시 생성
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (gStu) {
                                    await models_g.Student.update({ // state update 안됨
                                        term: student.term,
...
router.post('/permission/ajax/cancel_selection', async function(req, res, next) { //신청 배정현황의 학생 선택 취소버튼
    try {
