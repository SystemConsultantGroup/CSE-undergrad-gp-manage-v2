...
router.get('/main', async function (req, res, next) {
  try {
    var [loginLog, loginCnt, loginTodayCnt, users] = await Promise.all([
    ...
    loginLog.forEach(function (log) {
      log.time_ = moment(log.time).format('YYYY-MM-DD HH:mm:ss');
    });
    console.log(loginTodayCnt);

    var systems = await models.System.findAll();
    var unUserCnt = 0;
    users.forEach(function (user) {
      if (user.status == 1 || user.status == 3 || user.Student.SystemId == 1 || user.Student.SystemId == 13)
        unUserCnt++;
    });
...
router.post('/prof_list/ajax/get_profs', async function (req, res, next) {
  try {
    var assigned = await models.Student.findAll({
      where: {
        yearterm: {
          [Op.in]: [new Date().getFullYear(), new Date().getFullYear() + '01', new Date().getFullYear() + '02'],
        },
      },
      attributes: ['ProfId', [models.sequelize.fn('count', '*'), 'count']],
      group: 'ProfId',
    });
    var users = await models.Prof.findAll({
      include: [
        {
          model: models.User,
...
router.get('/student_list/excel/:id', async function (req, res, next) {
  try {
...
router.get('/prof/:id', async function (req, res, next) {
  try {
...
router.post('/prof/:id/ajax/get_students', async function (req, res, next) {
  try {
    var users = await models.User.findAll({
      where: {
        type: 2,
...
router.get('/prof_login/:id', async function (req, res, next) {
  // 귀찮아서 위 소스 복사함
  try {
    var user = await models.User.findOne({
      where: {
...
router.get('/prof_register', async function (req, res, next) {
  try {
    var users = await models.User.findAll({
      where: {
        type: 1,
...
router.get('/prof_register/:id', async function (req, res, next) {
  // 귀찮아서 위 소스 복사함
  try {
    var users = await models.User.findAll({
      where: {
...
router.post('/prof_register/ajax/get_prof', async function (req, res, next) {
  try {
    var user = await models.User.findOne({
      where: {
        id: req.body.id,
...
router.post('/prof_register', async function (req, res, next) {
  try {
    if (req.body.id) {
      // 수정일경우
      var user = await models.User.findOne({
...
router.post('/prof_register/ajax/del_prof', async function (req, res, next) {
  try {
    var user = await models.User.findOne({
      where: {
        id: req.body.id,
...
router.all('/prof_excel_save', async function (req, res, next) {
  try {
    var users = await models.User.findAll({
      where: req.body.arr
        ? {
...
//------------------------------------------------------------------------------------------
router.get('/student_list', async function (req, res, next) {
  try {
    var systems = await models.System.findAll();
  ...
  try {
    ...
    if (user) {
      ...
      res.render('cssys/work/admin/student_view', {
        ...
        state: [
          user.Student.dataValues.state % 10,
          parseInt((user.Student.dataValues.state % 100) / 10),
          parseInt(user.Student.dataValues.state / 100),
        ],
      });
    } else next();
  } catch (err) {
...
router.post('/student/:id', upload.single('upload'), async function (req, res, next) {
  try {
    var user = await models.User.findOne({
      where: {
        type: 2,
    ...
    if (user) {
      if (req.body.delete) {
        if (user.Student[req.body.delete].id == 1) {
          ...
          try {
            await storage.removeStoredFile(user.Student[req.body.delete].path);
          } catch (e) {
            // ignore
          }
          await user.Student[req.body.delete].destroy();
          res.send({
            result: true,
          });
        }
        ...
        var file = req.file;
        var objectKey = storage.makeObjectKey(['work', req.body.upload], file.originalname);
        ...
        if (user.Student[req.body.upload]) {
          try {
            await storage.removeStoredFile(user.Student[req.body.upload].path);
          } catch (e) {
            // ignore
          }
          await user.Student[req.body.upload].destroy();
          await user.Student['set' + req.body.upload.charAt(0).toUpperCase() + req.body.upload.slice(1)](studentfile);
          res.send({
            result: true,
          });
        } else {
...
router.get('/student_login/:id', async function (req, res, next) {
  // 귀찮아서 위 소스 복사함
  try {
    var user = await models.User.findOne({
      where: {
...
router.get('/student_register', async function (req, res, next) {
  try {
    var users = await models.User.findAll({
      order: [['name', 'ASC']],
      include: [models.Prof],
...
router.get('/student_register/:id', async function (req, res, next) {
  // 귀찮아서 위 소스 복사함
  try {
    var users = await models.User.findAll({
      order: [['name', 'ASC']],
...
router.post('/student_register/ajax/get_student', async function (req, res, next) {
  try {
    var user = await models.User.findOne({
      where: {
        id: req.body.id,
  ...
  try {
    ...
    if (req.body.id) {
      ...
      if (user !== null) {
        ...
        await user.update(req.body);
        await user.Student.update(req.body);
        var stu = await models_g.Student.findOne({
          where: {
            UserId: user.Student.UserId,
          },
        });
        if (stu) {
          await models_g.Student.update(
            {
              term: req.body.term,
      ...
      if (user === null) {
        ...
        req.body.state = 0;
        var newUser = await models.User.create(req.body);
        await newUser.createStudent(req.body);
        var gUser = await models_g.User.findOne({
          where: {
            ids: req.body.ids,
          },
        });
...
router.post('/student_register/ajax/del_student', async function (req, res, next) {
  try {
    ...
    if (user) {
      ...
      await models.StudentFile.destroy({
        where: {
          UserId: user.id,
        },
      });
  ...
  try {
    ...
    for (const data of obj[0].data) {
      if (data[0]) {
        ...
        if (!errFlag) {
          // Look up prof by name if provided
          var profId = null;
          if (data[17]) {
          ...
          if (user === null) {
            ...
            try {
              ...
              try {
                ...
                var newStudent = await newUser.createStudent(userTmp.Student);
                await models_g.Student.create(userTmp.Student);
                insertCount++;
              } catch (errors) {
                await newUser.destroy();
            ...
            try {
              ...
              try {
                ...
                // g_student 만들기
                var gStu = await models_g.Student.findOne({
                  where: {
                    UserId: user.id,
                  },
                });
...
router.all('/student_excel_save', async function (req, res, next) {
  try {
    var users = await models.User.findAll({
      where: req.body.arr
        ? {
    ...
    users.forEach(function (user) {
      data.push([
        ...
        user.Student.title,
        ['미심사', '심사통과', '재심대상자', '기합격'][user.Student.result],
        user.Student.comment,
        user.Student.gryearterm,
        user.Student.System ? user.Student.System.phase : '(미등록)',
        ['아니오', '예'][user.Student.islock ? 1 : 0],
        user.Student.yearterm,
        user.Student.Prof ? user.Student.Prof.User.name : '',
        user.Student.StudentInfoId ? '제출' : '',
        user.Student.oathId ? '제출' : '',
        !user.Student.proposalId
          ? ''
          : user.Student.state % 10 == 2
            ? '반려'
            : user.Student.state % 10 == 1
              ? '승인'
              : '제출(확인 필요)',
        !user.Student.midreportId
          ? ''
          : parseInt((user.Student.state % 100) / 10) == 2
            ? '반려'
            : parseInt((user.Student.state % 100) / 10) == 1
              ? '승인'
              : '제출(확인필요)',
        !user.Student.finalreportId
          ? ''
          : parseInt(user.Student.state / 100) == 2
            ? '반려'
            : parseInt(user.Student.state / 100) == 1
              ? '승인'
              : '제출(확인필요)',
        user.Student.paperworkId ? '제출' : '',
        user.Student.presentationId ? '제출' : '',
        user.Student.conferenceId ? '제출' : '',
      ]);
    });
    var buffer = xlsx.build([
      {
...
router.post('/system', async function (req, res, next) {
  try {
    var systems = await models.System.findAll();
...
router.post('/permission/ajax/get_permissions', async function (req, res, next) {
  try {
...
router.post('/permission/ajax/cancel_selection', async function (req, res, next) {
  //신청 배정현황의 학생 선택 취소버튼
  try {
...
router.post('/paperwork/ajax/get_paperworks', async function (req, res, next) {
  try {
    var users = await models.User.findAll({
      where: {
        type: 2,
...
router.get('/paperwork/application/:id', async function (req, res, next) {
  try {
    var user = await models.User.findOne({
      where: {
        type: 2,
...
router.post('/paperwork/application/change', async function (req, res, next) {
  try {
