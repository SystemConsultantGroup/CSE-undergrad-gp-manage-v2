        ...
        function (index) {
          if (user.Student[index])
            user.Student[index].link =
              '/cssys/work/ajax/file/download/' +
              index +
              '/' +
              user.Student[index].path.split('\\').reverse()[0].split('/').reverse()[0];
        },
      );
...
async function check_system() {
  ...
  for (let index = 0; index < work_system.length; index++) {
    if (
      (work_system[index].dataValues.id > 2 && work_system[index].dataValues.id < 13) ||
      work_system[index].dataValues.id > 13
    ) {
    ...
    if (user && user.Student && user.Student.System) {
      if (user.Student.status === 0 || user.Student.status === 2) {
        if (user.Student.islock) {
          ...
          (current_system[15] && user.Student.System.id == current_system[15].reupload) ||
          (current_system[17] && user.Student.System.id == current_system[17].reupload) ||
          (current_system[19] && user.Student.System.id == current_system[19].reupload)
        ) {
          // 재업로드 기간인 경우 2019.10.3 조건희
          let reupload_id;
        ...
        } else if (new Date() < user.Student.System.start || new Date() > user.Student.System.end) {
          // 현재 단계의 기간이 아님
      ...
      } else if (user.Student.status === 1) {
        // 휴학생 페이지
        res.render('cssys/work/student/system_status_1');
      } else if (user.Student.status === 0) {
        // 13 단계 재학 학생
    ...
    if (user && user.Student && user.Student.System) {
      ...
      if (user.Student.status === 1) {
        ...
        if (user.Student.StudentInfo) {
          ...
          if (user.Student.System.id >= 3 && user.Student.System.id <= 5) {
            //희망교수선택 기간중인지
            res.render('cssys/work/student/system_phase_2', {
              //수정 페이지
              user: user,
              student: user.Student,
              system: user.Student.System,
            ...
            res.render('cssys/work/student/system_phase_2_view', {
              //보기전용 페이지
              user: user,
              student: user.Student,
            });
...
// 서약서 및 제안서 제출 처리 페이지
router.post(
  '/system/proc/oath_proposal',
  upload.fields([
    { name: 'oath', maxCount: 1 },
      ...
      if (user && user.Student && user.Student.System) {
        if (
          (user.Student.System.id == 9 &&
            new Date() > user.Student.System.start &&
            new Date() < user.Student.System.end) ||
          (current_system[15] && current_system[15].reupload == 9)
        ) {
...
router.post(
  '/system/proc/midreport',
  upload.fields([{ name: 'midreport', maxCount: 1 }]),
  async function (req, res, next) {
    try {
      const current_system = await check_system();
      ...
      if (user && user.Student && user.Student.System) {
        if (
          (user.Student.System.id == 10 &&
            new Date() > user.Student.System.start &&
            new Date() < user.Student.System.end) ||
          (current_system[17] && current_system[17].reupload == 10)
        ) {
      ...
      if (user && user.Student && user.Student.System) {
        if (
          (user.Student.System.id == 11 &&
            new Date() > user.Student.System.start &&
            new Date() < user.Student.System.end) ||
          (current_system[19] && current_system[19].reupload == 11)
        ) {
          var result = { result: true };
          ...
          // Cleanup temp files
          try {
            if (finalreportFile) fs.unlinkSync(finalreportFile.path);
          } catch (err) {}
          try {
            if (paperworkFile) fs.unlinkSync(paperworkFile.path);
          } catch (err) {}
          try {
            if (presentationFile) fs.unlinkSync(presentationFile.path);
          } catch (err) {}
          try {
            if (conferenceFile) fs.unlinkSync(conferenceFile.path);
          } catch (err) {}
