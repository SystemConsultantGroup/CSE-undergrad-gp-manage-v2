...
router.all('/main/ajax/set_event', async function (req, res, next) {
  try {
    ...
    if (post !== null) {
      ...
      if (share !== null) {
        // 존재하면 공유가 되있음
        var targetShare = await models.Share.findOne({
          // 포스트 오버라이드 위해서
          where: {
            id: req.body.ShareId,
            UserId: req.session.user.id,
    ...
    if (share === null) {
      ...
      if (share.Calendar.UserId == req.session.user.id) {
        ...
        for (var index in share.Calendar.Shares) {
          if (share.Calendar.Shares[index].UserId != req.session.user.id)
            nowUsers.push(share.Calendar.Shares[index].UserId);
        }
...
router.post('/calendar/ajax/get_calendar', async function (req, res, next) {
  try {
    ...
    if (share !== null) {
      ...
      for (var index in share.Calendar.Shares) {
        if (share.Calendar.Shares[index].UserId != req.session.user.id)
          tmpArr.push(share.Calendar.Shares[index].UserId);
      }
      res.send({
        share: share,
        users: tmpArr,
