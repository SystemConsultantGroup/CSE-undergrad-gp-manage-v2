...
function normalizeUploadedFileName(originalName) {
  const input = String(originalName || 'file');
  // Normalize path separators so Windows-style paths (e.g. "C:\\fakepath\\foo.pdf")
  // are handled correctly on POSIX, and strip ASCII control characters.
  const normalizedInput = input.replace(/\\/g, '/').replace(/[\x00-\x1F\x7F]/g, '');
  let rawName = path.basename(normalizedInput) || 'file';

  // Avoid reserved or empty names.
  if (!rawName || rawName === '.' || rawName === '..') {
    rawName = 'file';
  }
  if (/[가-힣]/.test(rawName)) return rawName;

  try {
    const decodedName = Buffer.from(rawName, 'latin1').toString('utf8');
    if (decodedName && !decodedName.includes('\uFFFD') && /[가-힣]/.test(decodedName)) {
      return decodedName;
    }
  } catch (e) {}
...
function makeObjectKey(parts, originalName) {
  ...
  const uploadRoot = String((config.cssys && config.cssys.upload_path) || '')
    .replace(/\\/g, '/')
    .replace(/^\/+|\/+$/g, '');
  const rawName = normalizeUploadedFileName(originalName);
